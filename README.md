...
JSON 档案可直接复制备份，也可重新导入；保存的是聊天文字、时间、参与者和附件路径，**不复制照片/视频等附件原文件**。AI 结果和扫描出的照片日历仍需要使用各自导出按钮另存。档案为未加密的本地 JSON，不会上传 GitHub。可通过 `--data-dir /你的目录` 指定其他保存位置；这只切换目录，不会搬移旧档案。
