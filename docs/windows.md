...
下载成功的 **Windows EXE installer** 构建中的 `WeChatMemory-Installer-x64` 附件，解压后运行 `WeChatMemory-Setup-0.3.0-x64.exe`。支持 Windows 10 1903+ / Windows 11 x64，需 .NET Framework 4.8；安装到当前用户的应用目录，无需管理员权限，不修改系统 PATH。升级前关闭旧版窗口或控制台，在同一目录安装即可；聊天及已保存凭据保留。

DeepSeek Key 可在「AI 分析」点击「加密保存 Key」，以后输入框留空即可使用，重启应用不需要重填。卸载也保留该凭据；若不再使用，请先在界面删除。密文受当前 Windows 账户保护，不是明文配置文件。

安装后打开桌面 **WeChat Memory**，直接进入独立应用窗口，不打开浏览器，不需要保留黑色控制台。首次提取时，在 Windows 微信登录自己的账号，再点击窗口顶部「初始化微信」或从开始菜单打开 **Initialize WeChat**，根据独立交互窗口提示初始化（只有访问微信进程确需权限时才以管理员身份运行该快捷方式）。关闭应用时会确认并停止属于本窗口的后台进程和未完成提取任务，不影响其他实例。

安装包内置 Python、Node.js 及固定版本提取组件。桌面界面使用微软 WebView2 Runtime；多数电脑已有，缺失时安装程序运行微软签名的引导程序联网安装，网络受限时可自行安装微软 WebView2 Runtime 后重试。云端 AI 请求仍需手动确认。包尚未代码签名，可能提示未知发布者，请核对私有仓库来源和同包 SHA256，不要关闭安全软件。卸载保留 Documents/WeChatMemory 内的聊天、加密凭据、桌面 WebView2 配置目录及提取工具用户配置。

CI 使用虚构记录验证安装、独立运行环境、DPAPI 跨进程读取、原生窗口加载、示例聊天渲染、分析页切换、文件导出、关闭后停止服务及卸载保留数据。该自动化验收不等于真实微信账号提取验证或全面视觉验收。
